# oscmix_gui/main.py
# Updated based on analysis of oscmix C source code, for packaging, and with Tabs.
import queue
import sys
import threading
import time
...
try:
    ...
    from PySide6.QtWidgets import QTabWidget  # Added for Tabs
...
# Volume/Link controls for playback seem handled via /mix/... paths, not added here yet.
...
# --- Channel Counts (Adjust if your setup differs or oscmix exposes differently) ---
NUM_INPUT_CHANNELS = 8
NUM_OUTPUT_CHANNELS = 8
NUM_PLAYBACK_CHANNELS = 8  # Adjust based on your system / oscmix capabilities

# GUI Update Rates
METER_GUI_UPDATE_RATE_HZ = 60  # Update GUI meters N times per second
...
class OSCHandler(QObject):
    ...
    meter_received = Signal(
        str, int, float
    )  # type ('input'/'output'/'playback'), ch_idx, value
    gain_received = Signal(
        str, int, float
    )  # type ('input'/'output'), ch_idx, value (dB)
    link_received = Signal(
        str, int, bool
    )  # type ('input'/'output'), ch_idx (odd), value
    ...
    def _map_osc_addresses(self):
        ...
        print(
            f"Mapping Inputs : {OSC_INPUT_GAIN.format(ch='<ch>')}, {OSC_INPUT_STEREO.format(ch='<ch>')}, {OSC_INPUT_LEVEL.format(ch='<ch>')}"
        )
        print(
            f"Mapping Outputs: {OSC_OUTPUT_VOLUME.format(ch='<ch>')}, {OSC_OUTPUT_STEREO.format(ch='<ch>')}, {OSC_OUTPUT_LEVEL.format(ch='<ch>')}"
        )
        print(f"Mapping Playback: {OSC_PLAYBACK_LEVEL.format(ch='<ch>')} (Meters only)")
        ...
        for i in range(1, NUM_PLAYBACK_CHANNELS + 1):
            ch_idx_0_based = i - 1
            self.dispatcher.map(
                OSC_PLAYBACK_LEVEL.format(ch=i),
                self._handle_meter,
                "playback",
                ch_idx_0_based,
            )
            # Add mappings here if playback volume/link OSC paths are found in oscmix

    # --- Handlers for specific OSC messages ---
    def _handle_meter(self, address, *args):
        self.message_received_signal.emit(address, list(args))
        if len(args) >= 3:
            osc_type = args[0]  # 'input', 'output', or 'playback'
            ch_idx = args[1]
            value = args[2]  # Use the first float value for the meter
    ...
    def _handle_gain(self, address, *args):  # Handles input gain & output volume
        self.message_received_signal.emit(address, list(args))
        if len(args) >= 3:
            osc_type = args[0]  # 'input' or 'output'
            ch_idx = args[1]
            value = args[2]  # dB value
            if isinstance(value, (float, int)):
                QMetaObject.invokeMethod(
                    self,
                    "emit_gain_received",
    ...
    def _handle_link(self, address, *args):  # Handles input & output link
        self.message_received_signal.emit(address, list(args))
        if len(args) >= 3:
            osc_type = args[0]  # 'input' or 'output'
            ch_idx = args[1]  # Odd channel index
            value = args[2]  # 0 or 1
...
class OscmixControlGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.widgets = {
            ...
            "playback_gain": [],
            "playback_link": [],
            "playback_meter": [],  # Added playback widgets
        }
    ...
    def init_ui(self):
        """Creates and arranges all the GUI widgets, now using Tabs."""
        main_widget = QWidget()
        ...
        # ... (Connection group layout remains the same as before) ...
        ...
        main_layout.addWidget(self.tab_widget)  # Add tabs below connection group
        ...
        # --- Populate Input Tab ---
        input_tab_layout = QVBoxLayout(self.input_tab)  # Layout for the input tab page
        input_group = QGroupBox("Hardware Inputs")  # GroupBox inside the tab
        input_tab_layout.addWidget(input_group)
        input_grid = QGridLayout()  # Grid layout for the groupbox content
        ...
        for i in range(NUM_INPUT_CHANNELS):
            ...
            gain_slider.setRange(0, 750)  # 0.0 to 75.0 dB
        ...
        playback_group = QGroupBox("Software Playback Channels")
        playback_tab_layout.addWidget(playback_group)
        playback_grid = QGridLayout()
        playback_group.setLayout(playback_grid)

        # Add Header Row for Playback
        playback_grid.addWidget(
            QLabel("Meter"), 0, 0, 1, NUM_PLAYBACK_CHANNELS, alignment=Qt.AlignCenter
        )
        # Add headers for Volume/Link if controls are added later
        # playback_grid.addWidget(QLabel("Volume (dB)"), 2, 0, 1, NUM_PLAYBACK_CHANNELS, alignment=Qt.AlignCenter)
        # playback_grid.addWidget(QLabel("Link"), 4, 0, 1, NUM_PLAYBACK_CHANNELS, alignment=Qt.AlignCenter)

        # Create widgets for each playback channel (Meters only for now)
        for i in range(NUM_PLAYBACK_CHANNELS):
            ...
            meter_bar.setOrientation(Qt.Vertical)
            meter_bar.setFixedHeight(100)
            meter_bar.setToolTip("Playback Level")
            meter_bar.setObjectName(
                f"playback_meter_{i}"
            )  # Need object name if we add controls later

            # Placeholder: Add Volume slider and Link checkbox if OSC paths are found
            # gain_slider = QSlider(Qt.Vertical); gain_slider.setObjectName(f"playback_gain_{i}") ...
            # link_cb = QCheckBox(); link_cb.setObjectName(f"playback_link_{i}") ...

            playback_grid.addWidget(meter_bar, 1, i, alignment=Qt.AlignCenter)  # Row 1
            # playback_grid.addWidget(gain_slider, 3, i, alignment=Qt.AlignCenter) # Row 3 (if added)
            # playback_grid.addWidget(link_cb, 5, i, alignment=Qt.AlignCenter) # Row 5 (if added)
            playback_grid.addWidget(ch_label, 6, i, alignment=Qt.AlignCenter)  # Row 6

            # Store references to the widgets
            # self.widgets['playback_gain'].append(gain_slider) # If added
            # self.widgets['playback_link'].append(link_cb) # If added
            self.widgets["playback_meter"].append(meter_bar)

        # --- Populate Output Tab ---
        output_tab_layout = QVBoxLayout(self.output_tab)
        output_group = QGroupBox("Hardware Outputs")
        ...
        for i in range(NUM_OUTPUT_CHANNELS):
            ch_label = QLabel(f"{i+1}")
            gain_slider = QSlider(Qt.Vertical)
            gain_slider.setRange(-650, 60)  # -65.0 to +6.0 dB
        ...
        # ... (Log group layout remains the same as before) ...
    ...
    def connect_signals(self):
        ...
        # OSC Handler -> GUI Updates (Connections unchanged)
        ...
        self.osc_handler.meter_received.connect(
            self.on_meter_received
        )  # Handles input/output/playback
        self.osc_handler.gain_received.connect(
            self.on_gain_received
        )  # Handles input/output
        self.osc_handler.link_received.connect(
            self.on_link_received
        )  # Handles input/output

        # GUI -> OSC Sending (Connections unchanged)
        self.connect_button.clicked.connect(self.connect_osc)
        self.disconnect_button.clicked.connect(self.disconnect_osc)

        # Connect sliders and checkboxes for inputs and outputs
        for i in range(NUM_INPUT_CHANNELS):
        ...
        # Add connections for playback controls here if they are added to the UI later
            ...
            if meter_widget_list:
                for ch_idx, value in meter_values.items():
                    if 0 <= ch_idx < len(meter_widget_list):
                        meter_widget_list[ch_idx].setValue(
                            float_to_slider_linear(value)
                        )
            # Clear stored values for this type for the next interval
            meter_values.clear()

    @Slot(str, int, float)
    def on_gain_received(self, type, ch_idx, value_db):
        """Updates gain/volume slider (handles input/output)."""
        widget_key = f"{type}_gain"  # 'input_gain' or 'output_gain'
        slider_list = self.widgets.get(widget_key)
        if slider_list and 0 <= ch_idx < len(slider_list):
            ...
            # Add handling for 'playback' if volume controls are added later
            slider.blockSignals(False)

    @Slot(str, int, bool)
    def on_link_received(self, type, ch_idx, value):
        """Updates link checkbox (handles input/output)."""
        widget_key = f"{type}_link"  # 'input_link' or 'output_link'
    ...
    def send_gain_from_gui(self, slider_value):
        ...
        # Add handling for 'playback' if volume controls are added later
        # elif type == 'playback':
        #     osc_address = OSC_PLAYBACK_VOLUME.format(ch=ch_num_1_based)
        #     float_value_db = slider_to_playback_volume(slider_value) # Need conversion func
        else:
            return
    ...
    def send_link_from_gui(self, state):
        ...
        # Add handling for 'playback' if link controls are added later
        # elif type == 'playback':
        #      osc_address = OSC_PLAYBACK_STEREO.format(ch=ch_num_1_based)
        else:
            return
    ...
    def _update_linked_channel_state(self, type, odd_ch_idx, is_linked):
        ...
        # Use generic 'gain' key as object names cover input gain & output volume
        gain_list = self.widgets.get(f"{type}_gain")
        # Add check for playback if controls are added later
